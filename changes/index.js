...
var printmaster = require('./printmasterlist.js');
module.exports = {
	...
	download_masterlist : function(req,res){
		async.auto({
			production : function(cb){
				var content = {};
				content.table = "productions";
			...
			printTofile : ['combination',function(cb,cres){
				...
				var files = new Array();
				for(var i=0; i<cres.combination.length; i+=80){
					var content = new Array();
					...
					printmaster(content,function(err,file){
						console.log("printed");
						console.log(content);
						files.push(file);
						if(files.length >= cres.combination.length){
