...
//	doc.image('./routes/Template3.PNG', 0, 0,{width: 615, height: 467});
	...
	var boxNo = 0;
	doc.fontSize(12);
	doc.text("Production",30,yaxis+10);
	doc.text("Date",45,yaxis+20);
	doc.text("Category",90,yaxis+10); //30
	doc.text("Production",140,yaxis+10); //80
	doc.text("Id",160,yaxis+20)
	doc.text("Series",200,yaxis+10); //140
	//doc.text("Production",175,yaxis+30);
	//doc.text("Date",190,yaxis+40);
	doc.text("Box",235,yaxis+10);
	doc.text("Card Number Combination",290,yaxis+10);
	doc.fontSize(10);
	doc.registerFont('Cardo', 'fonts/FreeSerif.ttf')
	doc.font('Cardo');
	doc.text("_________________________________________________________________________________________________________",30,yaxis+25)
	for(var i =0; i<content.length; i++){
		yaxis += 10;
		doc.fontSize(9);
		doc.text(content[i].prod_date,40,yaxis+25);
		doc.text(content[i].category,110,yaxis+25); //50
		doc.text(content[i].prodid,150,yaxis+25); //90
		doc.text(content[i].series,200,yaxis+25); //140
		//doc.text(content[i].prod_date,180,yaxis+50); //180
		doc.text(boxList[boxNo],240,yaxis+25); //240
		//doc.text("           ",130,yaxis+30,{width:35,align:'center'})
		for(var j = 0;j<m;j++){
			xaxis += 15;
			doc.text(content[i].box[j],xaxis+265,yaxis+25,{width:35,align:'center'});
		//	if(j != 5){
		...
		if(boxNo > 4){
			boxNo = 0;
			doc.text("_____________________________________________________________________________________________________________________",30,yaxis+25)
		}
	}
